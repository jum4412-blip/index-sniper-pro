...
LIVE_CONFIRM = "START_V51_VWAP_LIVE"
STATE_PATH_DEFAULT = "data/v51_vwap_top20_state.json"
JSONL_PATH_DEFAULT = "data/v51_vwap_top20_events.jsonl"
LOG_PATH_DEFAULT = "logs/v51-vwap-top20.log"

DEFAULT_TOP20_FALLBACK = [
...
def load_settings() -> V51Settings:
    load_dotenv(ROOT / ".env")
    override = tuple(s.strip().upper() for s in os.getenv("V51_SYMBOLS", "").split(",") if s.strip())
    return V51Settings(
        category=os.getenv("V51_CATEGORY", os.getenv("CATEGORY", "USDT-FUTURES")).strip(),
        margin_coin=os.getenv("V51_MARGIN_COIN", os.getenv("MARGIN_COIN", "USDT")).strip().upper(),
        margin_mode=os.getenv("V51_MARGIN_MODE", os.getenv("MARGIN_MODE", "crossed")).strip(),
        leverage=_int("V51_LEVERAGE", 3),
        dry_run=_bool(os.getenv("V51_DRY_RUN"), False),
        live_enabled=_bool(os.getenv("V51_LIVE_ENABLED"), False),
        live_confirm=os.getenv("V51_LIVE_CONFIRM", "").strip(),
        loop_seconds=_int("V51_LOOP_SECONDS", 10),
        candle_refresh_seconds=_int("V51_CANDLE_REFRESH_SECONDS", 60),
        universe_count=_int("V51_UNIVERSE_COUNT", 20),
        max_active_symbols=_int("V51_MAX_ACTIVE_SYMBOLS", 20),
        max_open_positions=_int("V51_MAX_OPEN_POSITIONS", 3),
        total_capital_ratio=_float("V51_TOTAL_CAPITAL_RATIO", 0.30),
        per_order_capital_ratio_cap=_float("V51_PER_ORDER_CAPITAL_RATIO_CAP", 0.003),
        max_order_notional_usdt=_float("V51_MAX_ORDER_NOTIONAL_USDT", 200.0),
        vwap_window_minutes=_int("V51_VWAP_WINDOW_MINUTES", 180),
        warmup_minutes=_int("V51_WARMUP_MINUTES", 10),
        band_std_mult=_float("V51_BAND_STD_MULT", 2.0),
        adx_period=_int("V51_ADX_PERIOD", 14),
        adx_max=_float("V51_ADX_MAX", 20.0),
        tp_pct=_float("V51_TP_PCT", 0.006),
        sl_pct=_float("V51_SL_PCT", 0.003),
        shock_window_seconds=_int("V51_SHOCK_WINDOW_SECONDS", 5),
        shock_pct=_float("V51_SHOCK_PCT", 0.15),
        shock_cooldown_minutes=_int("V51_SHOCK_COOLDOWN_MINUTES", 10),
        daily_target_pct=_float("V51_DAILY_TARGET_PCT", 2.0),
        daily_loss_pct=_float("V51_DAILY_LOSS_PCT", 1.0),
        max_trades_per_day=_int("V51_MAX_TRADES_PER_DAY", 30),
        state_path=os.getenv("V51_STATE_PATH", STATE_PATH_DEFAULT).strip(),
        jsonl_path=os.getenv("V51_JSONL_PATH", JSONL_PATH_DEFAULT).strip(),
        notify_every_minutes=_int("V51_NOTIFY_EVERY_MINUTES", 30),
        notify_actions_only=_bool(os.getenv("V51_NOTIFY_ACTIONS_ONLY"), True),
        use_coingecko=_bool(os.getenv("V51_USE_COINGECKO"), True),
...
def _instrument_map(client: BitgetUTAClient, settings: V51Settings) -> dict[str, dict[str, Any]]:
...
def resolve_universe(client: BitgetUTAClient, settings: V51Settings) -> tuple[list[str], dict[str, dict[str, Any]], dict[str, Any]]:
    ...
    if settings.symbols_override:
        ...
        if not candidates:
            source = "fallback"
            candidates = DEFAULT_TOP20_FALLBACK[: settings.universe_count]
    ...
    for sym in DEFAULT_TOP20_FALLBACK:
        if len(selected) >= settings.universe_count:
            break
        if sym in available and sym not in selected:
            selected.append(sym)
...
def _ticker_map(client: BitgetUTAClient, settings: V51Settings) -> dict[str, float]:
...
def _open_positions_map(client: BitgetUTAClient, settings: V51Settings) -> dict[str, dict[str, Any]]:
...
def _cancel_symbol_orders(client: BitgetUTAClient, settings: V51Settings, symbol: str) -> dict[str, Any]:
...
def _close_position(client: BitgetUTAClient, settings: V51Settings, pos: dict[str, Any], reason: str) -> dict[str, Any]:
    ...
    intent = OrderIntent(
        ...
        client_oid=f"v51close-{symbol.lower()}-{int(time.time())}",
    )
    res = client.place_order(intent, dry_run=settings.dry_run)
    return {"reason": reason, "symbol": symbol, "side": side, "qty": qty, "result": res}


def _per_order_capital_ratio(settings: V51Settings, selected_count: int) -> float:
...
def _should_notify(state: dict[str, Any], settings: V51Settings, force: bool = False) -> bool:
    if force:
        return True
...
def _summary_message(settings: V51Settings, result: dict[str, Any]) -> str:
    ...
    lines = [
        "🧲 <b>v5.1 Top20 VWAP Rubber Band</b>",
        f"상태: {result.get('status')} / 실주문: {'없음(DRY)' if settings.dry_run else '있음(LIVE)'}",
        f"일일손익: {float(result.get('day_pnl_pct') or 0):.3f}% / 목표 +{settings.daily_target_pct}% / 손실 -{settings.daily_loss_pct}%",
...
def preflight() -> dict[str, Any]:
    ...
    if not settings.dry_run:
        if not settings.live_enabled:
            errors.append("V51_LIVE_ENABLED=true required")
        if settings.live_confirm != LIVE_CONFIRM:
            errors.append(f"V51_LIVE_CONFIRM={LIVE_CONFIRM} required")
    if settings.total_capital_ratio > 0.30:
        errors.append("V51_TOTAL_CAPITAL_RATIO > 0.30 blocked in v5.1")
    if settings.leverage not in {1, 2, 3, 4, 5}:
        errors.append("V51_LEVERAGE must be 1~5")
    if len(universe) == 0:
...
def run_once(*, notify: bool = True) -> dict[str, Any]:
    ...
    if not settings.dry_run and (not settings.live_enabled or settings.live_confirm != LIVE_CONFIRM):
        ...
        if notify:
            _send(bot, "⛔ <b>v5.1 VWAP live blocked</b>\nV51_LIVE_ENABLED / V51_LIVE_CONFIRM 확인 필요")
        return result
    ...
    for sym in universe:
        ...
        for action, side, pos_side, entry_price in orders_to_place:
            ...
            if action == "LONG":
                ...
                tp = entry_price * (1.0 - settings.tp_pct)
                sl = entry_price * (1.0 + settings.sl_pct)
            oid = f"v51-{sym.lower()}-{action.lower()}-{int(time.time())}"[-32:]
            try:
    ...
    result["status"] = "ACTIONS" if result.get("actions") else "OBSERVE"
    append_jsonl(settings.jsonl_path, result)
    save_state(settings.state_path, state)
    if notify:
        force = bool(result.get("actions")) or not settings.notify_actions_only
        if force and _should_notify(state, settings, force=bool(result.get("actions"))):
...
def main() -> None:
    p = argparse.ArgumentParser(description="v5.1 Top20 VWAP Rubber Band scalp live bot")
