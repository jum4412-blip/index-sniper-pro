...
screen -dmS sniper-exec-dry bash -lc 'cd ~/index-sniper-pro && source venv/bin/activate && while true; do echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] starting strategy exec loop"; python main.py --mode strategy-exec-loop; code=$?; echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] strategy loop exited code=$code; restarting in 30s"; sleep 30; done >> logs/sniper-exec-dry.log 2>&1'
