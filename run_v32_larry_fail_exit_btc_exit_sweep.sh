...
set -euo pipefail
cd "$(dirname "$0")"
PYTHONPATH=$PWD python -m index_sniper.backtest.larry_fail_exit exit-sweep \
  ...
  --exit-modes "${BT_V32_EXIT_MODES:-target_reclaim_close,day_open_reclaim_close,next_open}"
